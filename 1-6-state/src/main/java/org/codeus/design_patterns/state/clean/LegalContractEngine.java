...
/**
 * Workflow engine for legal contracts with strict compliance requirements
 */
public class LegalContractEngine extends WorkflowEngine {
    ...
    public void configureWorkflow(DocumentContext context) {
        ...
        context.getReviewers().addAll(Arrays.asList("legal_reviewer_1", "legal_reviewer_2"));

        // Complex documents need senior review
        if (context.getContent() != null && context.getContent().length() > 10000) {
            context.getReviewers().add("senior_legal_reviewer");
        }

        context.getApprovers().add("legal_manager");

        // Urgent contracts need director approval
        if (context.isUrgent()) {
            context.getApprovers().add("legal_director");
        }

        // International contracts need compliance
        if (contentContains(context.getContent(), "international", "cross-border")) {
            context.getApprovers().add("compliance_officer");
        }
